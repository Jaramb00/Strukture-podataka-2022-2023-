...
StabloPos StvoriNewCvor(char *grad, int brojSt);
int PrintAll(Position head);
int InOrder(StabloPos root);
Position NadiPoImeuDrzave(char *drzava, Position head);
...
int main() {
	...
	do {
		...
		printf("Upisite drzavu koju zelite pretraziti:\n");
		InputString(drzava);
		current = NadiPoImeuDrzave(drzava, &Head);
		if (current != NULL) {
...
StabloPos r(char *grad, int brojSt)
{
	StabloPos q = NULL;
